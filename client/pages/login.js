...
function login(props) {
  ...
  const register = async () => {
    try {
      ...
      setLoading(false);
      const { email, photoURL, uid, displayName } = user?.user;
      const data = { email: email, img: photoURL, id: uid, name: displayName };
      props.setUser(data);
      Router.push("/");
    } catch (error) {
      setLoading(false);
  ...
  const login = async () => {
    try {
      ...
      setInputEmail("");
      setPassword("");
      const user = await signInWithEmailAndPassword(auth, inputEmail, password);
      setLoading(false);
      const { email, photoURL, uid, displayName } = user?.user;
      const data = { email: email, img: photoURL, id: uid, name: displayName };
      props.setUser(data);
      ...
      <div className="grid grid-cols-1 sml:grid-cols-6 h-5/6 w-11/12 lar:w-3/6 mid:w-4/6 ">
        ...
        <div className="p-4 sml:rounded-r-lg sml:rounded-l-none rounded-lg col-span-1  sml:col-span-4 grid grid-rows-8  bg-white">
          ...
          <div className="flex justify-center items-center rows-span-1">
            ...
            <div className="border max-w-fit border-gray-200 rounded-md p-3 cursor-pointer hover:scale-105 mx-2">
